...
public class PlaceTest {
    ...
    @Test
    public void placeEndpointPositive() {
        given().contentType(ContentType.JSON)
        .header("Authorization", "Bearer "+adminToken)
        .body("{\"location\":\"Halle-test\",\"placeNumber\":233}")
          .when().post("http://localhost:8080/places")
          .then()
             .statusCode(200);
    }

    @Test
    public void placeEndpointNegative() {
        given().contentType(ContentType.JSON)
          .when().get("http://localhost:8080/places/0")
          .then()
             .statusCode(400);
    }
}
